...
const EVENTS_DATA: Event[] = [
  ...
  // --- Original Events (Re-indexed to avoid ID conflict) ---
  {
    id: '101',
    ...
    price: 15000,
    image: 'https://images.unsplash.com/photo-1552674605-46d530064509?q=80&w=1000&auto=format&fit=crop',
  ...
  },
  {
    ...
    price: 0,
    image: 'https://images.unsplash.com/photo-1518577915332-c2a19f149a75?q=80&w=1000&auto=format&fit=crop',
    duration: '2 hrs',
...
export const EventsPage: React.FC<EventsPageProps> = ({ 
  ...
  useEffect(() => {
    if (initialEventId) {
        const eventFound = EVENTS_DATA.find(e => e.id === initialEventId);
        if (eventFound) {
  ...
  }, [searchTerm, selectedCategory, selectedDifficulty, selectedDuration, dateRange]);
                        ...
                        {Array.from({ length: getDaysInMonth(calendarViewDate.getFullYear(), calendarViewDate.getMonth()) }).map((_, i) => {
                            ...
                            return (
                                <button 
                                    ...
                                    className={`p-2 transition-all relative ${bgClass} ${textClass}`}
                                >
                                    {day}
                                </button>
                            );
